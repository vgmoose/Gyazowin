...
TCHAR *szTitle			= _T("Imgyazo");		// タイトル バーのテキスト
TCHAR *szWindowClass	= _T("IMGYAZOWIN");	// メイン ウィンドウ クラス名
TCHAR *szWindowClassL	= _T("IMGYAZOWINL");	// レイヤー ウィンドウ クラス名
HWND hLayerWnd;
...
BOOL uploadFile(HWND hwnd, LPCTSTR fileName)
{
	...
	buf << sBoundary;
	buf << sCrLf;
	buf << "content-disposition: form-data; name=\"image\"; filename=\"imgur.com\"";
	buf << sCrLf;
	...
	// User-Agentを指定
	const TCHAR* ua = _T("User-Agent: curl/7.30.0\r\n");
